...
import { PATIENT_NAV_ITEMS, type PatientNavIcon } from "@/constants/patient-navigation";
...
function isNavActive(pathname: string, href: string): boolean {
  ...
  if (href === ROUTES.patient.dashboard) {
    return pathname === ROUTES.patient.dashboard || pathname === `${ROUTES.patient.dashboard}/`;
  }
  return pathname === href || pathname.startsWith(`${href}/`);
}

function NavIcon({ name, className }: { name: PatientNavIcon; className?: string }) {
        ...
        >
          {showExpandedCopy ? (
            <div className="min-w-0 pl-0.5">
              ...
              <p className="truncate text-xs font-medium text-muted-foreground">{patientName}</p>
            </div>
          ) : (
